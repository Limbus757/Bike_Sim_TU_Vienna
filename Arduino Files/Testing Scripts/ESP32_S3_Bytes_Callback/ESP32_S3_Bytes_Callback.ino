...
void writeResistanceToTacx(uint16_t resistance) {
    if (!bleConnected || !fCPinit || !ControlPointChar) return;
...
void initFitnessMachineControlPoint() {
    ...
    // stage: purple (handshake in progress)
    rgbLedWrite(RGB_BUILTIN, RGB_BRIGHTNESS, 0, RGB_BRIGHTNESS); 


    // send control request(0x00)
...
class ClientCallbacks : public NimBLEClientCallbacks {
    ...
    bleConnected = false; 
    fCPinit = false;
    pIndoorBikeDataChar = nullptr; // Clear pointers
    pControlPointChar = nullptr;   // Clear pointers
    
    NimBLEDevice::deleteClient(p); // Wipe memory and free RAM
    pClient = nullptr;  // Reset global client pointer
    
    rgbLedWrite(RGB_BUILTIN, RGB_BRIGHTNESS, 0, 0); // Red LED
}
};
...
void connectToTacx() {
    ...
    for (int i = 0; i < results.getCount(); i++) {
        // FIX 1: Add 'const' here to match the library's return type
        const NimBLEAdvertisedDevice* device = results.getDevice(i); 
        
        if (device->getName() == TACX_NAME) {
            // STAGE: YELLOW (Found bike, attempting radio connection)
            rgbLedWrite(RGB_BUILTIN, RGB_BRIGHTNESS, RGB_BRIGHTNESS, 0); 
            
            pClient = NimBLEDevice::createClient();
            pClient->setClientCallbacks(new ClientCallbacks());
            
            // FIX 2: NimBLE's connect() accepts a const pointer, so this works now
            if (pClient->connect(device)) {
                // STAGE: BLUE (Radio connected, searching services)
                ...
                // Connect failed, revert to RED
                rgbLedWrite(RGB_BUILTIN, RGB_BRIGHTNESS, 0, 0);
            }
            break; // Found the bike, stop searching the results list
        }
    }
}
...
void bluetoothTask(void *pvParameters) {
    ...
    while (1) {
        uint32_t now = millis();
        if (!bleConnected) {
            rgbLedWrite(RGB_BUILTIN, RGB_BRIGHTNESS, 0, 0); //red when disconnected
            connectToTacx(); 
        } else if (fCPinit) {
