				...
				switch (Tools.current)
				{
					...
					case Tool.Scale:
						if (selectedAnchorPoint != -1)
						{
							...
							for (int i = -1; i <= 1; i += 2)
							{
								...
								{
									var fmh_246_50_638896684688071472 = Quaternion.identity; var nPos = Handles.FreeMoveHandle(hPos,
					...
					case Tool.Rect:
						...
						foreach (var p in points)
						{
							...
							{
								var fmh_298_87_638896684688099063 = Quaternion.identity; var newPos = Handles.FreeMoveHandle($"Handle-{p.index}".GetHashCode(), p.pos,
